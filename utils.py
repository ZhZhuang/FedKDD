...
# 实现中
# 希望得到 训练和测试同分布的情况
def dirichlet_split_noniid_pro(train_labels, test_labels, alpha, n_clients):
    '''
    参数为alpha的Dirichlet分布将数据索引划分为n_clients个子集
    '''
    n_classes = train_labels.max()+1
    ...
    #######################################
    # 每一类的标签放到class_idcs的对应元素中, np.argwhere函数 ?
    test_class_idcs = [np.argwhere(test_labels == y).flatten()
                  for y in range(n_classes)]
    # 记录每个K个类别对应的样本下标

    test_client_idcs = [[] for _ in range(n_clients)]
    # 记录N个client分别对应样本集合的索引
    for c, fracs in zip(test_class_idcs, label_distribution):
        # np.split按照比例将类别为k的样本划分为了N个子集
        # for i, idcs 为遍历第i个client对应样本集合的索引
        for i, idcs in enumerate(np.split(c, (np.cumsum(fracs)[:-1] * len(c)).astype(int))):
            test_client_idcs[i] += [idcs]

    test_client_idcs = [np.concatenate(idcs) for idcs in test_client_idcs]

    # 训练和测试集 同分布
    return client_idcs, test_client_idcs
...
def cifar_noniid_dirichlet( num_users, alpha = 1.0, path ="./data", download = True  ):
    ...
    train_labels = np.array(dataset.targets)
    test_labels = np.array(test_dataset.targets)
    client_idcs, client_test_idcs = dirichlet_split_noniid_pro(train_labels,test_labels, alpha, num_users)  # 上下比例不一定相同,日后再改
...
def cifar_noniid_byclass( num_class, path ="./data",
                  download = True):
...
# 将dataset随机划分为 num_users 份，得到数据的索引
