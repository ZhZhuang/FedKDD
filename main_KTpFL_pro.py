...
from core_kt_pfl_pro import  kt_pfl_train_pro
...
def load_models( config, logger ):
    ...
    path_data_file = os.path.join("data_gen", "img_fmnist_100t_alpha1.0_5k")
    ...
    ##########===========  公共数据集  ============########
    # path_data_public = os.path.join("data_gen", "img_cifar10_100t_noniid4_5k")
    path_data_public = os.path.join("data_public", config.public_path)        # 临时写法，
    # # dataset_public 只需要 data属性，代码历史遗留问题
    dataset_public = fileDataset(path_data_public,
                            is_gray=is_gray,
                            is_public=True
                            ).data
    print(type(dataset_public))
    print(dataset_public.shape)

    # 公共数据集
    # dataset_public = get_public_data(config.dataset_path,
    #                               config.public_num,
    #                               dataname=config.dataset)  # ( n,32,32,3) (n 28 28)

    return client_list, transform, data_list , dataset_public
...
def init():
    ...
    parser.add_argument("--name", type=str, default="KTpFL_pro", help="KTpFL | FedMD")
    ...
    parser.add_argument("--dataset", type=str, default="FashionMNIST", help="Cifar10/ FashionMNIST")
    ...
    # 数据增强
    parser.add_argument("--use_aug", type=str2bool,default=False, help="use data augment or not")
    # 数据，生成数据的路径
    parser.add_argument("--gen_path", type=str, default="gen_path", help="generate image path")
    parser.add_argument("--public_path", type=str, default="public_path", help="generate public image path")
    parser.add_argument("--n_block", type=int, default=10, help="n blocks of generate image")
...
def main( config, logger ):
    ...
    client_list, t_transform , data_list , dataset_public = load_models(config, logger)     # 获取 模型对象
