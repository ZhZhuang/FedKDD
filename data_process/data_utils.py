...
from data_process.sampling import mnist_iid, mnist_noniid, mnist_noniid_unequal
from data_process.sampling import cifar_iid, cifar_noniid
...
class TransformTwice:
    ...
    def __call__(self, inp):
        ...
        out1 = self.transform(inp)
        out2 = self.transform(inp)  # out1 和 out2 不一样 :RandomAffine
        return out1, out2


#　
class data_double(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.transform is not None:
            # X = Image.fromarray(X)    # transform 之用于图片
            X = self.transform(X)   # retun (out1, out2)
...
def getdataset(args,conf_dict):
    ...
    N_samples_per_class = conf_dict["N_samples_per_class"]      # 3

    public_classes = conf_dict["public_classes"]       #[ 0, 1, 2, 3, 4, 5, ...... , 9 ]
    private_classes = conf_dict["private_classes"]  # [0,2,20,63,71,82],

    # 载入数据集
    if args.dataset == 'cifar':
        X_train_public, y_train_public, X_test_public, y_test_public = get_dataarray(args,dataset='cifar10')     # np.array()
    ...
    if args.iid:
        X_train_private, y_train_private \
            = generate_partial_data(X=X_train_private, y=y_train_private,       #取[private_class]中的data
                                    ...
                                    verbose=True)       # 获取整个数据集中 属于 private_classes 相应的数据
        ...
        for index, cls_ in enumerate(private_classes):
            y_train_private[y_train_private == cls_] = index + len(public_classes) # public_classes = [0, 1, 2, ... , 9]
            y_test_private[y_test_private == cls_] = index + len(public_classes)
        del index, cls_     #释放不用的变量的内存

        print("训练数据集的类   数量 \n",pd.Series(y_train_private).value_counts())    #Series产生带标签的一维数组 ， value_count 会直接返回一个以元素为索引，出现次数为值的Series
        # 总类别
        ...
        X_train_private, y_train_private \
            = generate_partial_data(X=X_train_private, y=y_train_private,
                                    class_in_use=private_classes,
                                    verbose=True)       #获得属于 private_classes 中的数据
        ...
        # relabel the selected private data for future convenience
        for index, cls_ in enumerate(private_classes):
            y_train_private[y_train_private == cls_] = index + len(public_classes)      # numpy 操作 []
            y_test_private[y_test_private == cls_] = index + len(public_classes)
        del index, cls_
        ...
        # generate private data
        if args.dataset == 'cifar':
            users_index = cifar_noniid(y_train_private, N_parties)      # dict_user
    ...
    # [X_train_public, y_train_public,X_test_public, y_test_public]  来自ciiar10
    # public_dataset ：字典 x_train_public ,y_train_public
    # private_data : 数组 ，每个元素为字典 idx,x,y
    # toral_private_data : 字典，idx，x，y
    # private_test_data : 字典 x, y  共享test
...
def generate_partial_data(X, y, class_in_use = None, verbose = False):
    ...
    if class_in_use is None:
        ...
        idx = [y == i for i in class_in_use]         #6 x 50000 bool
        idx = np.any(idx, axis = 0)                  # 50000 bool 或运算
    X_incomplete, y_incomplete = X[idx], y[idx]      #见过类似操作 ，使用Boolean类型的数组挑选一维数组中的值
    if verbose == True:
...
def generate_bal_private_data(X, y, N_parties = 10, classes_in_use = range(11), 
    ...
    for cls in classes_in_use:
        idx = np.where(y == cls)[0]     # np.where()[0] 表示行索引，np.where()[1]表示列索引, 行和列组成对应的元素
        # 选择cls类
        idx = np.random.choice(idx, N_samples_per_class * N_parties,    #cifiar per class 500 ,so N_samples_per_class max is 50
                               replace = data_overlap)
        combined_idx = np.r_[combined_idx, idx]     # np.r_是按列连接两个矩阵，就是把两矩阵上下相加，要求列数相等。
