...
class RandAugment:
    def __init__(self, n, m, is_gray = False):
        self.n = n
        self.m = m      # [0, 30] in fixmatch, deprecated. 已弃用
        self.is_gray = is_gray
        self.augment_list = augment_list()
        
    def __call__(self, img):
        # 对于灰度图片 fashion mnist 会报错，需要转换
        if self.is_gray:
            img = img.convert('RGB')    # ！！！！
        ...
        # img = cv2.cvtColor(img,cv2.COLOR_RGB2GRAY) # 转为灰度图片,  error, 方法不行
        if self.is_gray:
            img = img.convert('L')  #转灰度图， Fashion mnist需要的操作
        return img
