...
class fileDataset(Dataset):
    ...
    def load_data(self):
        ...
        for img_file in self.img_files:
            ...
            if self.transform:
                img = self.transform(img)  # Apply any additional transformations
            img_array = np.array(img)
            images.append(img_array)
        images_array = np.array(images)
        if self.is_gray:
            assert images_array.shape[1:] == (28, 28), "The grayscale images must be of shape (28, 28)"
        else:
            assert images_array.shape[1:] == (32, 32, 3), "The color images must be of shape (32, 32, 3)"
        return images_array
    ...
    def __getitem__(self, idx):
        ...
        return img, target       # ToTensor()(img) 声明类 ，然后调用
    ...
    def save_image(self,srcfile, dstpath= "issue_data", verbose= False):
